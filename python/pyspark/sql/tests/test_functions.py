...
class FunctionsTestsMixin:
    ...
    def test_try_make_timestamp(self):
        ...
        actual = df.select(F.try_make_timestamp(df.year, df.month, df.day, df.hour, df.minute, df.second)).collect()
    ...
    def test_try_make_timestamp_ltz(self):
        ...
        df = self.spark.createDataFrame(data, ["year", "month", "day", "hour", "minute", "second", "timezone"])
        actual = df.select(F.try_make_timestamp_ltz(df.year, df.month, df.day, df.hour, df.minute, df.second, df.timezone)).collect()
    ...
    def test_try_make_timestamp_ntz(self):
        data = [(2024, 5, 22, 10, 30, 0)]
        df = self.spark.createDataFrame(data, ["year", "month", "day", "hour", "minute", "second"])
        actual = df.select(F.try_make_timestamp_ntz(df.year, df.month, df.day, df.hour, df.minute, df.second)).collect()
        self.assertEqual(actual, [Row(datetime.datetime(2024, 5, 22, 10, 30))])
